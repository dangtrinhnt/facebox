...
(function ($) {
	...
	function Wysiwyg() {
		...
		this.defaults = {
html: '<!DOCTYPE html><html style="margin:0"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body style="margin:0;">INITIAL_CONTENT</body></html>',
			debug: false,
			controls: {},
			css: {},
			events: {},
		...
		this.init = function (element, options) {
			...
			if (newX === 0 && element.cols) {
				newX = (element.cols * 8) + 100;
			}
			if (newY === 0 && element.rows) {
				newY = (element.rows * 16) + 200;
			}
			...
			if (this.options.iFrameClass) {
				...
				this.editor.css({
					minHeight: (newY+200).toString() + "px",
					// fix for issue 12 ( http://github.com/akzhan/jwysiwyg/issues/issue/12 )
					width: (newX > 50) ? (newX+200).toString() + "px" : ""
				});
				if ($.browser.msie && parseInt($.browser.version, 10) < 7) {
					this.editor.css("height", (newY+12).toString() + "px");
				}
			}
			/** 
			...
			if (!this.options.iFrameClass) {
				this.element.css({
					width: (newX > 0) ? (newX+205).toString() + "px" : "100%"
				});
			}
